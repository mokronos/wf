...
The workflow file is self-contained (imports only from `@mokronos/wfkit`), so it can be
...
```bash
wf create order-saga --file examples/order-saga/order.ts
wf run order-saga '{"apiUrl":"http://localhost:8788","orderId":"o-1","sku":"sku-widget","quantity":2,"unitPriceCents":250,"address":"42 Sunny Lane"}'

# compensation path (refund + release, non-zero exit):
wf run order-saga '{"apiUrl":"http://localhost:8788","orderId":"o-2","sku":"sku-widget","quantity":1,"unitPriceCents":250,"address":"1 Nowhere Street"}'
